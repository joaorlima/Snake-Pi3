...
void gameScreen()
{
    ...
    if (!game)
    {
      fill(255, 0, 0); rect(ax*20, ay*20, 20, 20); 
      
      if (frameCount % 5 == 0)
      {
          x.add(0, x.get(0) + dx[dir]); y.add(0, y.get(0) + dy[dir]);
          if(x.get(0) < 0 || y.get(0) < 0 || x.get(0) >= 30 || y.get(0) >= 30)
          {
              game = true;
          }
          ...
          { 
            ax = (int)random(0, 30); ay = (int)random(0, 30);
          }
...
void keyPressed()
{
    ...
    if(newdirs!=-1&&(x.size()<=1||!(x.get(1)==x.get(0)+dx[newdirs]&&y.get(1)==y.get(0)+dy[newdirs]))) 
    {
        dir=newdirs;
    }
    ...
    if(newdirk!=-1&&(x.size()<=1||!(x.get(1)==x.get(0)+dx[newdirk]&&y.get(1)==y.get(0)+dy[newdirk])))
    {
        dir=newdirk;
    }
